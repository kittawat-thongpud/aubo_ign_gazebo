...
bool AuboRos2Driver::start()
{
  ...
  send_service.robotServiceLeaveTcp2CanbusMode();
  ...
  if (ret1 == aubo_robot_namespace::InterfaceCallSuccCode)
  {
    ...
    if (ret2 == aubo_robot_namespace::InterfaceCallSuccCode)
    {
      if (state.isRealRobotExist)
      {
        ...
        send_service.robotServiceInitGlobalMoveProfile();
        /** Interface call: Set the maximum acceleration of the articulated motion ***/
        aubo_robot_namespace::JointVelcAccParam jointMaxAcc;
        jointMaxAcc.jointPara[0] = MAX_JOINT_ACC;
        jointMaxAcc.jointPara[1] = MAX_JOINT_ACC;
        jointMaxAcc.jointPara[2] = MAX_JOINT_ACC;
        jointMaxAcc.jointPara[3] = MAX_JOINT_ACC;
        jointMaxAcc.jointPara[4] = MAX_JOINT_ACC;
        jointMaxAcc.jointPara[5] = MAX_JOINT_ACC;   ////The interface requires the unit to be radians
        send_service.robotServiceSetGlobalMoveJointMaxAcc(jointMaxAcc);

        /** Interface call: set the maximum speed of articulated motion ***/
        aubo_robot_namespace::JointVelcAccParam jointMaxVelc;
        jointMaxVelc.jointPara[0] = MAX_JOINT_ACC;
        jointMaxVelc.jointPara[1] = MAX_JOINT_ACC;
        jointMaxVelc.jointPara[2] = MAX_JOINT_ACC;
        jointMaxVelc.jointPara[3] = MAX_JOINT_ACC;
        jointMaxVelc.jointPara[4] = MAX_JOINT_ACC;
        jointMaxVelc.jointPara[5] = MAX_JOINT_ACC;   ////The interface requires the unit to be radians
        send_service.robotServiceSetGlobalMoveJointMaxVelc(jointMaxVelc);

        int ret = send_service.rootServiceRobotControl(aubo_robot_namespace::RobotControlCommand::ClearSingularityOverSpeedAlarm);
  ...
  while (rclcpp::ok())
  {
    ...
    if (state.isMoving && rib_buffer_size_ < MINIMUM_BUFFER_SIZE)
    {
      if (joint_queue.size() > 0)
      {
        PlanningState ps;
        joint_queue.pop(ps);
        int ret = send_service.robotServiceFollowModeJointMove(ps.joint_pos);
        if(ret != aubo_robot_namespace::InterfaceCallSuccCode)
        {
            std::cerr<<joint_queue.size()<<"Joint failed.　ret:"<<ret<<"|"<<ps.joint_pos[0]<<"|"<<ps.joint_pos[1]<<"|"<<ps.joint_pos[2]<<"|"<<ps.joint_pos[3]<<"|"<<ps.joint_pos[4]<<"|"<<ps.joint_pos[5]<<std::endl;
            //handleArmStopped();
        }
