...
JointTrajectoryAction::JointTrajectoryAction(std::string controller_name,std::string node_name):Node(node_name)
{
  ...
  // Dynamic topic construction using robot_name
  std::string moveit_controller_topic = "/" + robot_name + "/moveit_controller";
  std::string fjt_feedback_topic = "/" + robot_name + "/fjt_feedback";
  std::string moveit_execution_topic = "/" + robot_name + "/moveit_execution";
  for (int i = 0; i < 6; i++)
  {
    RCLCPP_INFO(this->get_logger(), "joint name %d %s", i, joint_names[i].c_str());
  }

  moveit_controller_pub_ = this->create_publisher<trajectory_msgs::msg::JointTrajectoryPoint>(moveit_controller_topic, 5000);
  fjt_feedback_sub_ = this->create_subscription<control_msgs::action::FollowJointTrajectory_Feedback>(
    fjt_feedback_topic, 10, std::bind(&JointTrajectoryAction::fjtFeedbackCallback, this, _1));
  moveit_execution_sub_ = this->create_subscription<std_msgs::msg::String>(
    moveit_execution_topic, 10, std::bind(&JointTrajectoryAction::moveitExecutionCallback, this, _1));
...
void JointTrajectoryAction::watchDogTimer()
{
  if (has_active_goal_)
  {
      ...
      RCLCPP_INFO(this->get_logger(), "Goal is active trajectory_state_recvd_ %d",trajectory_state_recvd_);
...
void JointTrajectoryAction::calculateMotionTrajectory()
{
  ...
  for (int i = 0; i < motion_size - 1; i++)
  {
    ...
    while (tt < toSec(current_goal_point.time_from_start))
    {
      ...
      tt += 1.0 / 200; //discrete time period in sec
